...
export default function ActivePlan() {
  return (
    <>
      {/* Desktop view */}
      <div className="hidden md:block mt-6 w-[80%] shadow-md">
        ...
        <table className="w-full">
          ...
          <tbody className="text-black/50">
            {finance.map((item) => (
              <tr key={item.id}>
                <div className="flex flex-col justify-start items-start gap-2 py-2  px-4">
                  <td className="">{item.name}</td>
                  <td className="text-sm ">{item.start}</td>
                </div>
